...
<head>
  ...
  <style>
    ...
    .field-label { font-weight: 600; color: #667eea; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }
    .field-value { background: white; padding: 12px 16px; border-radius: 8px; border-left: 4px solid #667eea; font-size: 15px; }
    .message-field .field-value { white-space: pre-wrap; line-height: 1.8; background: #f0f4ff; border-left-color: #764ba2; }
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="field">
        ...
        <div class="field-value"><a href="mailto:${escapeHtml(formData.email)}" style="color: #667eea; text-decoration: none;">${escapeHtml(formData.email)}</a></div>
      </div>
      
      ${formData.phone ? `
      <div class="field">
        <div class="field-label">📱 Phone</div>
        <div class="field-value"><a href="tel:${escapeHtml(formData.phone)}" style="color: #667eea; text-decoration: none;">${escapeHtml(formData.phone)}</a></div>
      </div>
      ` : ''}
    ...
    <div class="footer">
      ...
      <p style="margin: 8px 0 0 0; opacity: 0.7;">From AUST BIC Contact Form</p>
    </div>
  </div>
</body>
</html>
    ...
    form.append('_template', 'table'); // Use FormSubmit default template, we'll override with html
    form.append('_html', 'on'); // Enable HTML emails
